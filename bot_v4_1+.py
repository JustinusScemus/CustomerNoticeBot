...
async def sort_notice(company:Company):
    try: 
        notices = await company.findnotice()
    except TimeoutError:
...
async def write_json(old_file, new_file, t, company:Company) -> tuple[dict, set]:
    print('working')
    notices = await sort_notice(company)
...
async def probe_(textchannel: dc.TextChannel, error_channel: dc.TextChannel, company:Company, thread: int = 1):
    ...
    new_json, new_set = await write_json(old_file, new_file, t, company)
...
def initialize_file(company : Company):
    ...
    for file in datafiles:
        ...
        print(f'{needfile} initialized.', end='\t')
        f.close()
    print()
...
def run_discord_bot():
    ...
    async def probes_(text_channel, error_channel):
        ...
        s(Citybus.routeslist)
        s(KMBus.routeslist)
        await aio.gather(
            probe_(text_channel, error_channel, Citybus),
            probe_(text_channel, error_channel, KMBus),
    ...
    async def update_bravo_routes(textchannel: dc.TextChannel, error_channel: dc.TextChannel):
        ...
        datestamp = dt.now(tz(td(hours=+8))).strftime("%Y-%m-%d")
        ...
        if len(new_route_set) + len(removed_routes_set) == 0:
            message = Citybus.circles(3) + f"CNB V{BOT_VERSION}: Routes stay the same for {Citybus.displayname}, checked: {datestamp}"
            await textchannel.send(message)
        else:
          if len(new_route_set) > 0:
            message = ":yellow_circle:" * 5
            message += f"CNB V{BOT_VERSION}: New route for bravovus: {new_route_set}, checked: {datestamp}\n@everyone"
            await error_channel.send(message)
          if len(removed_routes_set) > 0:
            message = Citybus.circles(5) + f"CNB V{BOT_VERSION}: Citybus removed: {removed_routes_set}, checked: {datestamp}\n@everyone"
            await error_channel.send(message)
    ...
    async def update_kmb_routes(textchannel: dc.TextChannel, error_channel: dc.TextChannel):
        ...
        if len(new_route_set) + len(removed_routes_set) == 0:
            message = KMBus.circles(3) + f"CNB V{BOT_VERSION}: Routes stay the same for {KMBus.displayname}, checked: {datestamp}"
            await textchannel.send(message)
        else:
          if len(new_route_set) > 0:
            message = ":red_circle:" * 5
            message += f"CNB V{BOT_VERSION}: New route for KMBLWB: {new_route_set}, checked: {datestamp}\n@everyone"
            await error_channel.send(message)
          if len(removed_routes_set) > 0:
            message = KMBus.circles(5) + f"CNB V{BOT_VERSION}: KMBLWB removed: {removed_routes_set}, checked: {datestamp}\n@everyone"
            await error_channel.send(message)
